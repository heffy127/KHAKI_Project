 ...
 /*  alert(naver_id_login.oauthParams.access_token); */
  ...
  function naverSignInCallback() {
    ...
    $.ajax({ // 소셜 로그인에 id가 저장되어 있는지 조회
		url: "checkSocialMypage.do?social_id=" + naver_id + "&social_type=naver",
		success: function(result){
			var check = result // already가 아닌경우 sessionId값을 가져옴
			if(check.trim() == 'already'){ // 해당 네이버 계정으로 다른 아이디 소셜 계정 연결
				...
				$.ajax({
					url: "mypage_socialInsert.do?id=" + check + "&social_id=" + naver_id + "&social_type=naver",
					success: function(result){
